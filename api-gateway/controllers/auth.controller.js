...
export const investorLogin = async (req, res) => {
  ...
  try {
    ...
    findUserByWallet(wallet_address, async (err, user) => {
      ...
      if (!user) {
        if (!username) return res.status(400).json({ message: 'Investor name is required' });

        // Auto-register the investor
        await query(
          "INSERT INTO users (wallet_address, role, username) VALUES ($1, $2, $3)",
          [wallet_address, 'investor', username]
        );
        user = { wallet_address, role: 'investor', username };
        console.log(`Auto-registered investor: ${username} (${wallet_address})`);
      } else {
        if (user.role !== 'investor') {
          return res.status(403).json({ message: 'Access denied: Not an investor user' });
        }
